...
def explain_rise(image_path,model,N,fit_callback,exp_callback,device):
    ...
    with torch.no_grad():
        ...
        controllo = torch.nn.functional.softmax(output,dim=1)[0]
    print('Predicted Class: ',pred_class )
    print('Controllo con softmax: ',controllo)
    print('Shape, min, max: ',img.shape, img.min(), img.max()) 
    ...
    plt.imshow(saliency[pred_class],cmap='jet',alpha=0.7,aspect='auto')
    colorbar = plt.colorbar(orientation='vertical', shrink=1)
    colorbar.set_ticks(colorbar.get_ticks())  # Per applicare modifiche ai tick
    colorbar.ax.tick_params(axis='y', colors='white')  # Cambia il colore del testo dei tick in bianco
    plt.title('Saliency Map', fontsize=12, color='white')
...
if __name__  == '__main__':
    ...
    #explain_gradcam(image_path='./case_studies/1/1495.jpg',model=model,device=device)
    explain_rise(image_path='./case_studies/1/1495.jpg',model=model,N=100,device=device,fit_callback=fit_callback,exp_callback=explain_callback)
